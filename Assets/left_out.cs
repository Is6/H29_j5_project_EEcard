...
public class left_out : MonoBehaviour {
    ...
    void Start () {
        ...
        d = GameObject.Find("Canvas/MainMenu/Window/Left/Background/Label");//
	...
	void Update () {
        ...
        if (Input.anyKeyDown && a == 0 && c == 0)
        {
            ...
            g.text = "Left : " + k; //ここで変更表示

            if (Input.GetKey(KeyCode.KeypadEnter) && a == 0 && another_d[0] != k && another_d[1] != k && another_d[2] != k)//テンキーのエンター入力で保存
            {
                a = 1;//
                
                b = 0;
                key.GetComponent<config_d>().le_d();//設定保存
            }
        }
    }
